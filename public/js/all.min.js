...
angular.module('myApp').factory('MiparlFactory',	function($resource){
	var factory = {
		get : fnGet,
		save : fnSave,
		update : fnUpdate,
...
(function() {
	...
	Controller.$inject=['$http','$scope', '$routeParams', '$location', 'LaudoFactory', 'TipoDispositivoFactory', 'PerguntaFactory', 'ClienteFactory', 'MaquinaFactory', 'LimiteMaquinaFactory', 'InstalacoesDispEletricoFactory', 'ManualFactory', 'MiparlFactory', 'NormaTecnicaFactory', 'NormaRegFactory', 'PerigoFactory', 'RiscoFactory', '$uibModal', '$document',
    '$filter'];

	function Controller ($http, $scope, $routeParams, $location, LaudoFactory, TipoDispositivoFactory, PerguntaFactory, ClienteFactory, MaquinaFactory, LimiteMaquinaFactory, InstalacoesDispEletricoFactory, MiparlFactory, ManualFactory, NormaTecnicaFactory, NormaRegFactory, PerigoFactory, RiscoFactory, $uibModal, $document, $filter){
...
///////////////////MIPARL/////
		...
		self.btnIncluirMip = true;

		self.changeMip = function(){
			self.btnIncluirMip = self.miparlSelec ? false : true;
		}

		function buscaMiparl(){
			MiparlFactory.get().then(function(resp){
				if(resp.data){
					console.log(resp.data);
					self.miparls = resp.data;
					for(var x = 0; x < self.laudo.miparls.length; x++){
						for(var i = 0; i< self.miparls.length; i++){
							if(self.laudo.miparls[x].id == self.miparls[i].id){
								self.miparls.splice(i,1);
							}
						}
					}
				}else{
		...
		self.adicionaMiparl = function() {
			for(var i = 0; i < self.miparls.length; i++){
				if(self.miparls[i].id == self.miparlSelec.id){
					self.miparls.splice(i, 1);
				}
			};
			self.laudo.miparls.push(self.miparlSelec);
			self.btnIncluirMip = true;
			self.miparls.Mips = self.laudo.miparls;
		};

		self.removeMiparl = function(mip){
			for(var i = 0; i < self.laudo.miparls.length; i++){
				if(self.laudo.miparls[i].id == mip.id){
					self.laudo.miparls.splice(i,1);
					self.miparls.push(mip);
				}
			};
		};
////////////////////////////
...
(function() {
	...
	Controller.$inject=['$routeParams', '$resource', '$location', 'MiparlFactory'];

	function Controller($routeParams, $resource, $location, MiparlFactory){

		var self = this;
		self.init = function(){

			if($routeParams.miparlId){
				MiparlFactory.get($routeParams.miparlId)
				.then(function(resp){
					if(resp.data){
		...
		function save(){
			MiparlFactory.save(self.miparl)
			.then(function(resp){
				  if(resp.data){
		...
		self.remove = function(miparl){
			bootbox.confirm({
				...
				callback: function(confirma){
					if (confirma) {
						MiparlFactory.remove(self.miparl.id)
		...
		function update(){
			...
			MiparlFactory.update(self.miparl.id, self.miparl)
			.then(function(){
...
(function() {
	...
	Controller.$inject=['MiparlFactory'];

	function Controller(MiparlFactory){
		...
		self.remove = function(miparl){
		  bootbox.confirm({
			  ...
			  callback: function(confirma){
					if (confirma){
						MiparlFactory.remove(miparl.id)
							.then(function(data){
								buscaMiparl();
							});
						}
...
(function() {
	...
	Controller.$inject=['$routeParams', '$resource', '$location', 'ManualFactory'];
	...
	function Controller($routeParams, $resource, $location, ManualFactory){
		...
		self.remove = function(manual){
			bootbox.confirm({
				...
				callback: function(confirma){
					if (confirma) {
						ManualFactory.remove(self.manual.id)
...
(function() {
	...
	Controller.$inject=['$routeParams', '$resource', '$location', 'InstalacoesDispEletricoFactory'];

	function Controller($routeParams, $resource, $location, InstalacoesDispEletricoFactory){
		...
		self.remove = function(instalacoesDispEletrico){
			bootbox.confirm({
				...
				callback: function(confirma){
					if (confirma) {
						InstalacoesDispEletricoFactory.remove(self.instalacoesDispEletrico.id)
